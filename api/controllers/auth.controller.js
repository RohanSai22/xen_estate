...
export const google = async (req,res,next)=>{
    try{
       const user= await User.findOne({email:req.body.email})//mistake
       if(user){
        //if both are correct then we need to authenticate the user which is done using adding a cookie inside browser[create hashed token of email/id and save this token inside browser cookies]
        const token=jwt.sign({ id: user._id },process.env.JWT_SECRET)
        const { password : pass , ...rest }= user._doc;//to only send / show required part
       res.cookie('access_token',token,{ httpOnly: true}).status(200).json(rest);
       }
       else{
        //we need to generate a password for them for first time 
